...
export default function SuccessStep({
  ...
  const bankCode = paymentConfig.bank_code || 'TPB';
  const accountNumber = paymentConfig.account_number || '98668397979';
  const qrTemplate = paymentConfig.qr_template || 'compact2';
  const accountName = paymentConfig.account_name || 'THACH BAO LOC';
  const transferNote = orderCode || `${paymentConfig.transfer_note_prefix || 'ORDER'}0000`;
  const qrUrl = `https://img.vietqr.io/image/${bankCode}-${accountNumber}-${qrTemplate}.png?amount=${totalPrice}&addInfo=${encodeURIComponent(transferNote)}&accountName=${encodeURIComponent(accountName)}`;
  ...
  const handleDownloadQR = async () => {
    try {
      ...
      link.href = url;
      link.download = `QR_ThanhToan_${transferNote}.png`;
  ...
  const orderDisplay = orderCode || 'ORDER0000';
      ...
      <div className="flex flex-col lg:flex-row gap-5 mt-5">
          ...
          {isPaid ? (
            <div className="mb-8 bg-[#fdfdfd] border border-green-500/30 rounded-2xl p-5 text-center shadow-sm">
              ...
              <p className="text-[14px] text-slate-600 mb-8">
                Đơn hàng <strong className="text-slate-800 text-[15px]">{orderDisplay}</strong> đã được thanh toán hoàn tất.
              </p>
              ...
              <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 mb-8 max-w-lg mx-auto text-left">
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <div className="text-[13px] text-slate-600 leading-relaxed">
                      Chúng tôi đã gửi Voucher/Vé điện tử cùng hướng dẫn sử dụng chi tiết qua email
                      <strong className="text-slate-800">{contactInfo.email || 'nguyenvankien@gmail.com'}</strong>.
                      Vui lòng kiểm tra hộp thư và cả thư rác/spam.
                    </div>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-[13px] text-slate-600 leading-relaxed">
                      Dịch vụ sẽ được gửi trong vòng 24h.
                    </div>
                  </div>
                </div>
              </div>
                ...
                <ul className="space-y-3">
                  <li className="flex items-start gap-3">
                    ...
                    <div className="text-[15px] text-slate-800 font-medium">
                      Mã đơn hàng của bạn là <span className="font-bold">{orderDisplay}</span>
                    </div>
                  </li>
                  <li className="flex items-start gap-3">
                    <AlertCircle className="w-5 h-5 text-[#ff5b00] shrink-0 mt-0.5" />
                    <div className="text-[15px] text-slate-800">
                      Đây là đơn hàng chưa được thanh toán. Vui lòng thanh toán sớm để tránh bị hủy hoặc hết vé.
                    </div>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="w-5 h-5 text-[#65b035] shrink-0 mt-0.5" />
                    <div className="text-[15px] text-slate-800">
                      Voucher/Vé và hướng dẫn sử dụng sẽ được gửi đến <span className="font-bold">{contactInfo.email || 'nguyenvankien@gmail.com'}</span>
                    </div>
                  </li>
                </ul>
              </div>
                ...
                <div className="border border-[#ff5b00]/30 rounded-lg bg-white overflow-hidden mb-3 shadow-[0_2px_10px_rgb(0,0,0,0.02)]">
                  ...
                  <div className="p-4">
                    <p className="text-[13px] text-slate-600 mb-4 leading-relaxed">
                      Vui lòng chuyển đúng số tiền vào tài khoản bên dưới. Nội dung chuyển khoản phải là <strong>{transferNote}</strong>.
                    </p>
                    ...
                    <div className="bg-[#fff9e6] border border-[#ffecb3] p-4 rounded-md mb-6 inline-block w-full">
                      <div className="text-[14px] text-slate-800">
                        Vui lòng ghi rõ nội dung thanh toán: <span className="font-bold">{transferNote}</span>
                      </div>
                    </div>

                    <div className="flex flex-col md:flex-row gap-4">
                      <div className="flex-1 border border-slate-200 rounded-lg p-5 flex items-start gap-4">
                        <div className="w-[100px] h-[44px] rounded bg-slate-100 flex items-center justify-center font-bold text-slate-600">
                          TPBANK
                        </div>
                        <div className="text-[13px] text-slate-700 space-y-1.5 flex-1">
                          <div>
                            <span className="text-slate-500 min-w-[70px] inline-block">Ngân hàng:</span>
                            <strong className="text-slate-800"> {paymentConfig.bank_name || 'TPBank'}</strong>
                          </div>
                          <div>
                            <span className="text-slate-500 min-w-[70px] inline-block">Số tài khoản:</span>
                            <strong className="text-slate-800 text-[14px]"> {accountNumber}</strong>
                          </div>
                          <div>
                            <span className="text-slate-500 min-w-[70px] inline-block">Chủ tài khoản:</span>
                            <span>{accountName}</span>
                          </div>
                          <div>
                            <span className="text-slate-500 min-w-[70px] inline-block">Số tiền:</span>
                            <strong className="text-[14px] text-slate-800">{totalPrice.toLocaleString('vi-VN')}</strong>
                          </div>
                          <div>
                            <span className="text-slate-500 min-w-[70px] inline-block">Nội dung:</span>
                      ...
                      <div className="border border-slate-200 rounded-lg p-5 flex flex-col items-center justify-center bg-white min-w-[200px]">
                        <img
                          src={qrUrl}
                          alt="QR Code"
                          className="w-[150px] h-[160px] object-contain mix-blend-multiply cursor-pointer mb-3"
                          onClick={() => setIsPaid(true)}
                          title="Click để mô phỏng quét mã thành công"
                        />
                        <button
                        ...
                        <span className="text-[10px] text-slate-400 mt-3 text-center leading-tight">
                          *Click vào mã QR để mô phỏng<br />đã thanh toán thành công
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="bg-white border border-slate-200 rounded-lg p-4">
                ...
                <div key={item.id} className={`flex gap-4 ${index !== cartItems.length - 1 ? 'border-b border-slate-100 pb-5 mb-5' : ''}`}>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[13px] text-slate-800 font-medium">
                      Ngày sử dụng <span className="font-normal text-slate-600">{item.date}</span>
                    </div>
              ...
              <div className="mt-5 pt-5 border-t border-slate-200 flex justify-between items-center">
                ...
                <span className="font-bold text-slate-800 text-[18px]">
                  {totalPrice.toLocaleString('vi-VN')} <span className="underline text-[14px] ml-0.5 relative -top-0.5">đ</span>
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="w-full lg:w-[320px] shrink-0 space-y-6">
          ...
          <div className="bg-[#f0f9ff] border border-blue-100 rounded-xl p-5">
            <h3 className="font-bold text-[15px] text-slate-800 mb-4">Tại sao đặt vé với Di Vui</h3>
            <ul className="space-y-4">
              <li className="flex gap-3 text-[13px] text-slate-600">
                <CheckCircle2 className="w-4 h-4 text-[#ff5b00] shrink-0 mt-0.5" />
                <div>Di Vui cam kết chính sách giá tốt, thấp hơn giá mua trực tiếp!</div>
              </li>
              <li className="flex gap-3 text-[13px] text-slate-600">
                <CheckCircle2 className="w-4 h-4 text-[#ff5b00] shrink-0 mt-0.5" />
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            ...
            <div className="space-y-2 text-[13px] text-slate-600">
              <div className="flex justify-between gap-3"><span>Ngân hàng</span><span className="font-semibold text-slate-800">{paymentConfig.bank_name || 'TPBank'}</span></div>
              ...
              <div className="flex justify-between gap-3"><span>Nội dung</span><span className="font-semibold text-slate-800">{transferNote}</span></div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            ...
            <p className="text-[12px] text-slate-500 mb-4">
              Liên hệ ngay với các chuyên viên tư vấn du lịch của Di Vui để chúng tôi hỗ trợ bạn kịp thời
            </p>
            ...
            <div className="text-center text-[13px]">
              <span className="text-slate-500 italic">Hoặc tham khảo thêm</span>
              <br />
